...
class SetTeams extends React.Component {
    updateNumberOfTeams (value) {
      ...
      const numberOfWinnersError = this.props.game.winners.length - this.props.game.teams.numberOfTeams;
    ...
    updateTeams () {
      ...
      while (unassignedPlayers.length) {
        let unFilledTeams = Array.from(Array(this.props.game.teams.numberOfTeams).keys())
    ...
    render () {
      ...
      const randomiseButton = (
        <button
          onClick={() => this.props.onChangeTeams(this.updateTeams())}
        >
        Randomise Teams
        </button>
      );
      return (
        <div className="setTeams">
          ...
          <input
            ...
            onChange={(event) => this.props.onChangeTeams(this.updateNumberOfTeams(parseInt(event.target.value)))}
          >
          </input>
          {randomiseButton}
